		      ...
		      struct fuse_config *cfg)
{
    ...
    asprintf(&msg, "Protocol version: %d.%d\n", conn->proto_major,
                        conn->proto_minor);
    greenprint(msg);
    greenprint("Capabilities:\n");
    if(conn->capable & FUSE_CAP_POSIX_LOCKS)
        greenprint("\tFUSE_CAP_POSIX_LOCKS\n");
    if(conn->capable & FUSE_CAP_SPLICE_MOVE)
        greenprint("\tFUSE_CAP_SPLICE_MOVE\n");
    if(conn->capable & FUSE_CAP_SPLICE_READ)
        greenprint("\tFUSE_CAP_SPLICE_READ\n");
    if(conn->capable & FUSE_CAP_SPLICE_WRITE)
        greenprint("\tFUSE_CAP_SPLICE_WRITE\n");
    if(conn->capable & FUSE_CAP_READDIRPLUS)
        greenprint("\tFUSE_CAP_READDIRPLUS\n");
    if(conn->capable & FUSE_CAP_READDIRPLUS_AUTO)
        greenprint("\tFUSE_CAP_READDIRPLUS_AUTO\n");
		    ...
		    struct fuse_file_info *fi)
{
    ...
    greenprint( "read start\n");
	(void) path;
	res = pread(fi->fh, buf, size, offset);
	if (res == -1)
		res = -errno;
	char* string;
    asprintf(&string, "read end, errno=%d", res);
    greenprint( string);
	return res;
}
			...
			size_t size, off_t offset, struct fuse_file_info *fi)
{
	...
	char* string;
    asprintf(&string, "read_buf fi->fh=%ld, offset=%ld, want=%ld", fi->fh, offset, size);
    redprint(string);
